...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
        // RICH: pulled costly operation out of for loop. Only needs to assign once
        // This reduced time from 170ms to about 7ms!
     
    // RICH: Replaced all querySelectorAll with getElementsByClassName for speed purposes
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover');
    // Took this operation out of for loop. Major performance improvement.
    var winScroll = (document).body.scrollTop / 1250;
    
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(winScroll + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
    // RICH: calculate height of browser window to determine num of pizzas we really need
    // http://www.w3schools.com/jsref/prop_win_innerheight.asp
    var height = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
    var numPizzas = Math.ceil(height / s) * cols;
    console.log("Generating " + numPizzas + " pizzas.");
